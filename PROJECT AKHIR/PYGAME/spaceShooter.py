...
def main_menu():
    ...
    while True:
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            draw_text(screen, "Tekan [ENTER] Untuk Bermain", 30, WIDTH/2, HEIGHT/2)
            draw_text(screen, "Tekan [Q] Untuk Keluar", 30, WIDTH/2, (HEIGHT/2)+40)
            pygame.display.update()

    #pygame.mixer.music.stop()
    ready = pygame.mixer.Sound(path.join(sound_folder,'getready.ogg'))
    ready.play()
    title = pygame.image.load(path.join(img_dir, "getreadybg.png")).convert()
    title = pygame.transform.scale(title, (WIDTH, HEIGHT), screen)
...
bkgd = pygame.image.load(path.join(img_dir, 'starfield.jpg')).convert()
background_rect = bkgd.get_rect()
y=0
...
while running:
    ...
    #3 Draw/render
    rel_y=y2%bkgd.get_rect().width
    screen.blit(bkgd,(0,rel_y-bkgd.get_rect().width))
    screen.blit(bkgd,(800,rel_y-bkgd.get_rect().width))
    if rel_y<800:
        screen.blit(bkgd,(0,rel_y))
        screen.blit(bkgd,(800,rel_y))
